...
// - gets dimensions from source image if necessary
...
function makeImagePairData(i, largeImage) {
  ...
  // The size of the large image as specified on the image, or its true size.
  var lw = getInlineValue(largeImage, "width") || largeImage.naturalWidth;
  var lh = getInlineValue(largeImage, "height") || largeImage.naturalHeight;
  if (!lw || !lh) {
  ...
  // do this last, because sy may change if sh is changed
...
function initListeners() {
  if (document.addEventListener) {
    ...
    document.attachEvent('ontouchmove', touchTrack);
  }
  else {
    if (!document.all) {
      document.captureEvents(Event.MOUSEMOVE);
      document.captureEvents(Event.TOUCHMOVE);
    }
    document.onmousemove = mouseTrack;
    document.ontouchmove = touchTrack;
  }
}
...
function makeStatusString() {
  ...
  testImage(i);
  return "Number of magnifiers checked: " + imagePairData.length + "\n";
}
